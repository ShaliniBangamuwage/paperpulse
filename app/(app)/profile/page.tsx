...
import {
  FileText, Lightbulb, CheckCircle, Globe, Zap,
  Camera, Pencil, X, Save, Shield, Calendar,
  BookOpen, Bookmark, ExternalLink
} from 'lucide-react'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: userPapers } = await supabase.from('papers').select('*').eq('user_id', user.id).order('created_at', { ascending: false })
      const { data: saved } = await supabase.from('saved_ideas').select('*, idea:ideas(*)').eq('user_id', user.id)
  ...
  async function saveProfile() {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return
    const { error } = await supabase.from('profiles').update({ bio, display_name: displayName }).eq('id', user.id)
    if (!error) {
      setProfile((prev: any) => ({ ...prev, bio, display_name: displayName }))
      setEditing(false)
  ...
  const doneIdeas = savedIdeas.filter(s => s.status === 'done').length
  const publicPapers = papers.filter(p => p.is_public).length
  const initials = (profile?.display_name?.[0] || profile?.email?.[0] || '?').toUpperCase()

  const stats = [
    { label: 'Papers', value: papers.length, icon: <FileText size={18} /> },
    { label: 'Ideas', value: totalIdeas, icon: <Lightbulb size={18} /> },
    { label: 'Completed', value: doneIdeas, icon: <CheckCircle size={18} /> },
    { label: 'Public', value: publicPapers, icon: <Globe size={18} /> },
  ]
    ...
    <div className="min-h-screen dark:bg-gray-950 bg-gray-50 text-gray-900 dark:text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 pb-12">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-end gap-4 -mt-16">
              ...
              <div className="mb-1">
                ...
                <p className="dark:text-gray-400 text-gray-500 text-sm">{profile?.email}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {stats.map(stat => (
            <div key={stat.label} className="dark:bg-gray-900 bg-white border dark:border-gray-800 border-gray-200 rounded-2xl p-5 text-center shadow-sm">
          ...
          <div className="dark:bg-gray-900 bg-white border dark:border-gray-800 border-gray-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold dark:text-white text-gray-900">Recent papers</h2>
              </div>
            ...
            {papers.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="dark:text-gray-500 text-gray-400 text-sm">No papers yet</p>
              </div>
            ) : (
              <div className="space-y-2">
                {papers.slice(0, 4).map(paper => (
                  <div key={paper.id}
                    ...
                    className={`flex items-center gap-3 p-3 dark:bg-gray-800 bg-gray-50 rounded-xl ${paper.status === 'done' ? 'cursor-pointer hover:bg-orange-500/10 dark:hover:bg-orange-500/10' : ''} transition-colors`}>
                    <div className="w-8 h-8 bg-orange-500/20 rounded-lg flex items-center justify-center shrink-0">
                      <FileText size={14} className="text-orange-500" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium dark:text-white text-gray-900 truncate">{paper.title}</p>
                      <p className="text-xs dark:text-gray-500 text-gray-400">{new Date(paper.created_at).toLocaleDateString()}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${
                      paper.status === 'done' ? 'bg-green-500/20 text-green-400' :
                      paper.status === 'processing' ? 'bg-blue-500/20 text-blue-400' :
          ...
          <div className="dark:bg-gray-900 bg-white border dark:border-gray-800 border-gray-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold dark:text-white text-gray-900">Saved ideas</h2>
              </div>
            ...
            {savedIdeas.length === 0 ? (
              <div className="text-center py-8">
                <Bookmark size={32} className="mx-auto text-gray-300 dark:text-gray-700 mb-2" />
                <p className="dark:text-gray-500 text-gray-400 text-sm">No saved ideas yet</p>
              </div>
            ) : (
              <div className="space-y-2">
                {savedIdeas.slice(0, 4).map(item => (
                  <div key={item.id} className="flex items-center gap-3 p-3 dark:bg-gray-800 bg-gray-50 rounded-xl">
                    <div className="w-8 h-8 bg-orange-500/20 rounded-lg flex items-center justify-center shrink-0">
                      <Lightbulb size={14} className="text-orange-500" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium dark:text-white text-gray-900 truncate">{item.idea?.title}</p>
                      <p className="text-xs dark:text-gray-500 text-gray-400">{item.idea?.difficulty}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${
                      item.status === 'done' ? 'bg-green-500/20 text-green-400' :
                      item.status === 'building' ? 'bg-blue-500/20 text-blue-400' :
                      'bg-yellow-500/20 text-yellow-400'}`}>
                      {item.status}
                    </span>
                  </div>
